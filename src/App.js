...
const AppItem = props => {
  const {appDetails, setScore} = props
  const {thumbnailUrl, imageUrl, id} = appDetails
  ...
  return (
    <li className="app-item">
      <button className="images-buttons" type="button" onClick={sendId}>
        <img className="app-image" src={imageUrl} alt={thumbnailUrl} />
      </button>
    </li>
  )
}
...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      currentScore: 0,
      seconds: 60,
      getRandomNumber: [imagesList[Math.floor(Math.random() * 30) + 1]],
      isTimeUp: false,
    }
  }

  componentDidMount() {
    // Set the initial "match" image from the first object in imagesList
  ...
  setScore = id => {
    const {getRandomNumber} = this.state

    if (getRandomNumber[0].id === id) {
      // Update the score and randomize the "match" image
      this.setState(prevState => ({
        getRandomNumber: [imagesList[Math.floor(Math.random() * 30) + 1]],
  ...
  render() {
    ...
    const filteredApps = this.getActiveTabApps(imagesList)
    const firstImageUrl = getRandomNumber[0].imageUrl
    ...
    return (
      <div className="app-container">
        ...
        <div className={`app-store ${!isTimeUp ? 'flex' : 'none'}`}>
          <img
        ...
        <div className={`score-board ${isTimeUp ? 'flex' : 'none'}`}>
          <img
